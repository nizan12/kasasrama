...
export function SettingsPage() {
  ...
  const handleQrisUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (file.size > 2 * 1024 * 1024) { setQrisError("Ukuran gambar maksimal 2MB"); return; }
    ...
    reader.onload = async (ev) => {
      ...
      try {
        ...
        if (!validation.valid) {
          setQrisError(`QRIS tidak valid: ${validation.errors.join(", ")}`);
          return;
        }
  ...
  return (
    <div className="space-y-6 pt-12 lg:pt-0 max-w-2xl fade-in">
      {/* Title */}
      <div>
              ...
              {qrisImage && (
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100">
                  <div className="p-3 bg-white border border-slate-100 rounded-2xl inline-block flex-shrink-0">
                    <img src={qrisImage} alt="Static QRIS" className="w-28 h-28 object-contain" />
                  </div>
